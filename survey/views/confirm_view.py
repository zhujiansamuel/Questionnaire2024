...
class ConfirmView(TemplateView):
    ...
    def get_context_data(self,request, **kwargs):
        ...
        control_question_ = int(cache.get(control_question_key) or 0)
        if control_question_ >= 3:
            msg = "私たちは、あなたが他の人とは違う存在になるために、意図的に反対の選択肢を選んでいることを見抜いた。 \n\n 申し訳ございませんが、あなたのアカウントは停止され、あなたの回答はデータから削除されました。 "
            diagnostic_result_msg = "  "
            majority_rate_r = 0
            correctness_rate_r = 0
        else:
            valid_questions = int(kwargs.get("valid_questions", 0))
            msg, diagnostic_result_msg, majority_rate_r, correctness_rate_r = Diagnostic_Analyze(
                int(kwargs["majority_rate"]), int(kwargs["correctness_rate"]), valid_questions
            )
        context["msg"] = msg
